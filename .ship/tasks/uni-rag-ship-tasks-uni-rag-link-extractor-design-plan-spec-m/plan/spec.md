...
### AC-1: 网页链接提取
Given 用户粘贴一个普通网页 URL（如文章页面）
When 调用 `POST /api/ingest/url` 或 Web UI 提交
Then 系统必须提取正文内容、切块、向量入库，source 显示为页面标题
And 提取失败时必须返回明确的错误码（`timeout` / `blocked` / `no_content` / `network`）

### AC-2: 复用现有 pipeline
Given `LinkExtractionResult` 已生成
When 调用 `ingest_url()``
Then chunk、embed、vector、bm25 存储逻辑必须与 `ingest_file()` 完全一致
And metadata 中必须包含 `platform`、`source_url`、`content_type` 字段

### AC-3: Web UI 可用
Given 用户已打开 Web UI（`/`）
When 在链接输入框粘贴 URL 并点击"提取"
Then 必须显示进度条 → 完成后在文件列表中出现（标题代替文件名）
And 原有的文件上传区域必须保留不变

### AC-4: CLI 可用
Given 用户有有效的 Anthropic API key
When 运行 `uni-rag ingest-url <url>` 或 `uni-rag kb ingest-url <kb_id> <url>`
Then 必须正常完成提取入库流程
And 输出格式与 `uni-rag ingest` 一致

### AC-5: 向后兼容
Given 现有用户数据已存在
When 新版本部署后查询旧数据
Then 只读 `source`、`format`、`section`、`start`、`end` 的代码必须正常工作
And 新 metadata 字段对旧代码透明

### AC-6: 错误可观测
Given 提取器遇到不支持平台或网络错误
When 用户提交 URL
Then 必须返回用户可读的错误消息（映射表见 3.1）
And 错误消息应说明原因和建议操作

## 6. Golden Journeys

1. **J1 网页问答**：粘贴 arXiv 论文链接 → 提取入库 → 在 Web UI 提问 → 得到带引用的答案
2. **J2 CLI 批量**：`uni-rag ingest-url <url1>` → `uni-rag ingest-url <url2>` → `uni-rag ask "对比两篇"` → 跨文档回答
3. **J3 容错**：粘贴一个视频链接（无字幕）→ 收到 `no_content` 错误 → 用户理解原因
